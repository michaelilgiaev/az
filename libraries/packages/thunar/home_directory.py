...
This is a SUBMODULE of the thunar package (packages/thunar/home_directory.py): the folder
layout and Thunar's sidebar are built from the SAME list, so the data lives right next to the
sidebar code that consumes it. compiler._emit_homedir imports it as packages.thunar.home_directory.
...
file-manager sidebar can never drift -- packages/thunar/ imports LAYOUT from this module
...
# Thunar sidebar lists them (packages/thunar builds bookmarks from this list), so the
...
def resolved_home_path(rel_or_link_target: str) -> str:
    ...
    Used by packages/thunar to point each sidebar bookmark at the REAL location (so
...
def sidebar_entries() -> list[tuple[str, str]]:
    ...
    This is the single list packages/thunar turns into the GTK bookmarks file and Thunar
    ...
    runtime; see packages/thunar/live_sidebar.)
