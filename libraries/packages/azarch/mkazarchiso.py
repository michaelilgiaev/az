...
def profiledef_sh() -> str:
    """A minimal archiso profiledef naming the artifact azarch-headed-ssh-<ver>-x86_64.iso
...
def _overlay_sshd_variant(profile_dir: str, main_hash: str) -> None:
    ...
    (iso_name=azarch-headed-ssh), /etc/shadow (main hashed, root locked), and the
...
def cmd_mkazarchiso(args: list[str]) -> int:
    ...
    print(f"[OK] azarch-headed-ssh ISO written to {out_dir} -- log in as `main` with your "
          "--ssh password.")
    return 0
