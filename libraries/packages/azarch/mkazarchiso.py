...
def profiledef_sh() -> str:
    """A minimal archiso profiledef naming the artifact azarch-sshd-<ver>-x86_64.iso and
    locking down the security-sensitive modes. It REPLACES the releng profiledef (same
...
def _overlay_sshd_variant(profile_dir: str, main_hash: str) -> None:
    ...
    (iso_name=azarch-sshd), /etc/shadow (main hashed, root locked), and the sshd-hypervisor
    auto-setup service + its multi-user.target.wants enable link."""
...
def cmd_mkazarchiso(args: list[str]) -> int:
    ...
    print(f"[OK] azarch-sshd ISO written to {out_dir} -- log in as `main` with your "
          "--ssh password.")
    return 0
