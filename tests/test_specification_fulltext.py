...
an exception at generation time, so only a value assertion catches them:
  ...
  2. `LAYER_ROLE` -- the per-layer role strings. They are DERIVED from
     specification_svg.LAYER_DEFS (`f"{title} -- {subtitle}"`) precisely so the SVG and
     ...
     `LAYER_ROLE == [f"{t} -- {s}" for t, s in specification_svg.LAYER_DEFS]`, that it has
     the same length as LAYER_DEFS, and that `len(LAYER_ROLE) - 1` (the top layer
     index printed in every entry's `layer:` line) stays 6.

`_layer_of` is a thin re-export of specification_svg.layer_of, kept so the two modules
...
import specification_fulltext as F
...
# --- _layer_of: thin re-export of specification_svg.layer_of ------------------------

def test_layer_of_delegates_across_full_height_range():
    # _layer_of must return exactly what specification_svg.layer_of returns for every
    # height, so the SVG and the text file assign the same band.
    for h in range(0, 60):
        assert F._layer_of("pkg", "AnyCategory", h) == specification_svg.layer_of(
            "pkg", "AnyCategory", h
        )
